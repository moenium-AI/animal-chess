...
CANDIDATES = [
    ...
    (2, "ladder", "かいだんをつくる", "6k1/8/8/8/8/R7/1R6/7K w - - 0 1",
     "二枚のルックの階段。まず一枚で段を封鎖し、もう一枚を最終列へ回す二手の型。"),
    (2, "endgame", "たまをちかづけて", "6k1/8/5K2/8/8/8/8/R7 w - - 0 1",
     "ルックだけでは詰まない。まず自分の玉を近づけて逃げ道を消してから、ルックで最終列を封じる。"),

    # ---- 難易度アップ: 3手詰め(じっくり読む練習) ----
    (3, "endgame", "ルックでしとめる", "6k1/8/4K3/8/8/8/8/R7 w - - 0 1",
     "玉とルックだけの寄せを三手で。自分の玉を一歩ずつ近づけ、相手の玉を端へ追い込んでから封じる。"),
    ...
    (3, "ladder", "とおいかいだん", "3k4/8/8/8/8/8/R7/1R5K w - - 0 1",
     "離れた二枚のルックで階段を作る。まず段を封じ、交互に王手して三手で端まで押し込もう。"),
